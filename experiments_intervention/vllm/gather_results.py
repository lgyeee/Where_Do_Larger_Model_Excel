...
INTERVENTION_DIR = _SCRIPT.parents[1]
src_dir = _SCRIPT.parents[2] / "src"
...
def main():
    ...
    p.add_argument("--mode", choices=["slm-normal", "slm-guided", "llm-guided"], default="slm-normal")
    p.add_argument("--model_large", default="qwen3-32b")
    p.add_argument("--model_small", default="qwen3-8b")
    p.add_argument("--extraction_model", default="qwen3-32b")
    args = p.parse_args()
    ...
    for sid in range(args.num_shards):
        path = result_dir / f"{args.n_sample}_runs_shard{sid}.json"
        with open(path) as f:
            shard = json.load(f)
    ...
    out_path = result_dir / f"{args.n_sample}_runs.json"
